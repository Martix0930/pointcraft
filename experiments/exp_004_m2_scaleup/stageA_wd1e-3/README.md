...
- best ckpt by held-out strict IoU @ epoch 25, p=0.3
...
|--------|----|----------|--------------|-----|------|
| strict | 0.146 | 0.165 | **0.294** | 0.165 | ✅ |
| mid | 0.091 | 0.134 | **0.172** | 0.134 | ✅ |
| tolerant | 0.087 | 0.141 | **0.169** | 0.141 | ✅ |

- **VERDICT A** — real generalization: held-out > max(B1,B3) on all 3 cutoffs.
- strict ratios: 1.781× B3 shell, 0.388× of the 0.7576 recall ceiling
- per-train-tile strict IoU at best ckpt: {} (train≫held-out → memorized; both low → under-learned/bug)
- ⚠ **curve:** held-out strict peaks 0.294@ep25 then collapses to 0.173@ep200 while train IoU climbs -> overfits the train tiles; early-stop at the peak (more tiles / regularization).
